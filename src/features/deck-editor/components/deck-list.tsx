...
const count = (entries: EditorEntry[]) =>
  entries.reduce((total, entry) => total + entry.quantity, 0);

const byName = (a: EditorEntry, b: EditorEntry) => a.card.name.localeCompare(b.card.name);
...
export function DeckList({ hasCommander, flaggedCardIds }: DeckListProps) {
  ...
  const boards = useMemo(() => {
    const result: Record<DeckBoard, EditorEntry[]> = {
      commander: [],
      main: [],
      sideboard: [],
      maybeboard: [],
    };
    for (const entry of entries) result[entry.board].push(entry);
    for (const list of Object.values(result)) list.sort(byName);
    return result;
  }, [entries]);

  const groups = useMemo(() => {
    const result = new Map<CardCategory, EditorEntry[]>();
    for (const entry of boards.main) {
      const category = cardCategory(entry.card.typeLine);
      result.set(category, [...(result.get(category) ?? []), entry]);
    }
    return CARD_CATEGORIES.flatMap((category) => {
      const list = result.get(category);
      return list ? [{ category, entries: list }] : [];
    });
  }, [boards.main]);

  const rows = (list: EditorEntry[]) => (
  ...
  return (
    <div className="flex flex-col gap-6">
      {hasCommander && (
        <Zone title={DECK_BOARD_LABELS.commander} count={count(boards.commander)}>
      ...
      <Zone title={DECK_BOARD_LABELS.main} count={count(boards.main)}>
        {groups.length === 0 ? (
      ...
      {(["sideboard", "maybeboard"] as const).map(
        (board) =>
          boards[board].length > 0 && (
            <Zone key={board} title={DECK_BOARD_LABELS[board]} count={count(boards[board])}>
              {rows(boards[board])}
            </Zone>
          ),
      )}
